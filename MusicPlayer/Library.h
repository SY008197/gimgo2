...
private:                            
	...
	bool containsIgnoreCase(const std::string& text, const std::string& keyword) const; // 대소문자 구분 없이 포함 여부 확인 헬퍼 함수 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ string앞에 std추가
	...
	Playlist* findPlaylist(const std::string& playlistName); // 추가 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
	// 여기에는 const Playlist*로 했는데 Library.cpp 에서는 const 없는 Playlist 불러오려고해서 오류남 위에 코드 추가
